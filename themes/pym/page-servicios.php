				...
				<div class="col-xs-4">
					...
					<aside class="sidebarCommon">
						...
						<?php //Obtener los nombres y enlaces de los servicios 
							...
							$first_service = $services[0]; #var_dump($first_room);
							//variable de control 
							$i = 0; 
							if( !empty($services) ) : foreach( $services as $service ) :
						?> <!-- Botones -->
						<a href="<?= get_permalink($service->ID); ?>" class="<?= $i == 0 ? 'active' : '' ?>"><?php _e( $service->post_title , LANG ); ?>
							<!-- Icon  -->
						...
						<div class="row">
							<div class="col-xs-6 text-justify">
								...
								<?php 
									...
									echo apply_filters('the_content' , $first_service->post_content);
									else: 
